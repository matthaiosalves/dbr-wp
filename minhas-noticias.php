<?php
/*
Template Name: Minhas Noticias
*/

// Protege acesso: só usuários logados
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['post_id'], $_POST['action_type'])) {
  ...
  if ($is_admin || get_post_field('post_author', $post_id) == $user_id) {
    if ($is_admin) {
...
// Consulta todas as notícias do usuário (sem limite, paginação será via JS)
$args = [
  'post_type'      => 'noticia',
  'author'         => $user_id,
  ...
  <div class="col l8 s12" style="padding:0;">
    <div class="c_criacao">
      ...
      <div style="display:flex;align-items: center;">
        <p>
          Nesse espaço você pode visualizar todas as notícias que você fez aqui no DBR. Atenção:<br>
          Caso você for
          <strong>REDATOR:</strong> Você só verá suas notícias como
          <strong style="color:#BD2507">RASCUNHO</strong>, elas estão aguardando aprovação. Quando postadas o seu status será POSTADA. Os
          <strong>demais cargos</strong> podem
          <strong style="color:#BD2507">POSTAR</strong> e também
          <strong style="color:#BD2507">DESTACAR</strong> direto.
        </p>
      </div>

      <div style="margin:15px 0;">
        <a href="<?php echo esc_url(admin_url('post-new.php?post_type=noticia')); ?>" class="btn100 btn">
          Criar nova notícia
        </a>
      </div>
      ...
      <script>
        ...
        function renderNoticias() {
          ...
          subset.forEach(post => {
            ...
            <div class="noticia noticia-click" data-href="${post.permalink}" style="display:flex;align-items:flex-start;margin-bottom:20px;cursor:pointer;">
              ...
              <div style="width:130px;min-width:130px;">
                <a href="${post.edit_link}" class="btn btn100 transparent laranja" style="margin-bottom:10px;" onclick="event.stopPropagation();">Editar</a>
                ${renderAcoes(post)}
              </div>
            </div>`;
          });
          ...
          // Torna a div clicável (evitando cliques em formulário)
          setTimeout(function() {
            document.querySelectorAll('.noticia-click').forEach(function(div) {
              div.onclick = function(e) {
                // Se clicar em botão/form, não navega
        ...
        function renderAcoes(post) {
          ...
          if (isAdmin) {
            if (post.status === 'draft') {
              html += `<form method="post" onClick="event.stopPropagation();">
              <input type="hidden" name="post_id" value="${post.post_id}">
              <input type="hidden" name="action_type" value="publish">
              <button type="submit" class="btn btn100 transparent laranja" style="margin-bottom:8px;display:inline;">POSTAR</button>
            </form>`;
            } else {
              html += `<form method="post" onClick="event.stopPropagation();">
              <input type="hidden" name="post_id" value="${post.post_id}">
              <input type="hidden" name="action_type" value="draft">
              <button type="submit" class="btn btn100 transparent verde" style="margin-bottom:8px;display:inline;">POSTADA</button>
            </form>`;
            }
            if (!post.is_destacada) {
              ...
              <button type="submit" class="btn btn100 transparent laranja" style="margin-bottom:8px;display:inline;">DESTACAR</button>
            </form>`;
            } else {
              html += `<form method="post" onClick="event.stopPropagation();">
              <input type="hidden" name="post_id" value="${post.post_id}">
              <input type="hidden" name="action_type" value="remover_destaque">
              <button type="submit" class="btn btn100 transparent verde" style="margin-bottom:8px;display:inline;">DESTACADA</button>
            </form>`;
            }
            html += `<form method="post" onClick="event.stopPropagation();">
            <input type="hidden" name="post_id" value="${post.post_id}">
            <input type="hidden" name="action_type" value="delete">
            <button type="submit" class="btn btn100" style="background:#e96a6a;color:#fff;" style="margin-bottom:8px;display:inline;">Deletar</button>
          </form>`;
          } else {
            ...
            if (!post.is_destacada) {
              html += `<button disabled class="btn btn100 transparent laranja"DESTACAR</button>`;
            } else {
              html += `<button disabled class="btn btn100 transparent verde">DESTACADA</button>`;
            }
          }
          return html;
        }

        // Paginação "por demanda" (exibe máximo de 5 botões, navegação por setas)
